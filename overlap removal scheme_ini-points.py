...
dim = 3  # 2D or 3D case
rve_size, inc_size, inc_vf, inc_enlg = np.ones([dim])*200.0, 20., 0.60, 1.01   # rve_size, particle diameter, volume fraction, enlargement factor
...
alpha, tolerance = 0.375, (inc_size * 1.e-3) ** 2  # Step size for gradient descent, convergence tolerance
index = 0

# =========================== Initialization =========================
method = ['poisson', 'sobol', 'halton', 'thomas', 'bridson']
points = generate_points(method[index], inc_num, rve_size, dim)
...
# =========================== Save results ============================
np.savetxt(f"points{dim}d-{int(inc_vf*100)}.txt", all_points)